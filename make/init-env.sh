...
# Sign-in follows provider credentials rather than a separate feature flag.
# First-time interactive setup may fill them; later init keeps stored values.
fill_auth_providers() {
  local file="$1"
  ...
  if init_interviewing; then
    echo "Sign-in uses Google and Resend. Leave them empty to serve anonymous reading." >&2
    read -r -p "Configure sign-in providers? [n]: " answer
