...
grep -qx "CADDY_HOST_MATCHERS=localhost 127.0.0.1 localhost:8080 127.0.0.1:8080" "$tmp" \
  || fail "init must stamp unique localhost Caddy hosts"
...
PROJECT_NAME=labfresh POSTGRES_HOST_PORT=5444 REDIS_HOST_PORT=6391 \
  SHARED_ENV_FILE="$tmp" "$ROOT/make/init-env.sh" >/dev/null
grep -qx "COMPOSE_PROJECT_NAME=labfresh" "$tmp" || fail "PROJECT_NAME must stamp COMPOSE_PROJECT_NAME"
grep -qx "POSTGRES_HOST_PORT=5444" "$tmp" || fail "POSTGRES_HOST_PORT env must stamp the file"
