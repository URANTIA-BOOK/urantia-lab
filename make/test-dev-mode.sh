...
if grep -q 'apps:' "$ROOT/stack/db/postgres/docker-compose.dev.yml"; then
  fail "postgres dev overlay must not join apps"
fi
...
if grep -q 'apps:' "$ROOT/stack/redis/docker-compose.dev.yml"; then
  fail "redis dev overlay must not join apps"
fi
...
dev_net='name: ${COMPOSE_PROJECT_NAME:-urantialab}-dev'
for overlay in \
  "$ROOT/stack/db/postgres/docker-compose.dev.yml" \
  "$ROOT/stack/redis/docker-compose.dev.yml" \
  "$ROOT/stack/api/docker-compose.dev.yml" \
  "$ROOT/stack/hub/docker-compose.dev.yml"
do
  rel="${overlay#"$ROOT"/}"
  grep -q -- '- dev' "$overlay" \
    || fail "$rel must attach the default dev network"
  grep -Fq "$dev_net" "$overlay" \
    || fail "$rel must declare $dev_net"
done
grep -q -- '- backend' "$ROOT/stack/db/postgres/docker-compose.dev.yml" \
  || fail "postgres overlay networks: replaces the base list; keep backend"
grep -q -- '- backend' "$ROOT/stack/redis/docker-compose.dev.yml" \
  || fail "redis overlay networks: replaces the base list; keep backend"
grep -q -- '- apps' "$ROOT/stack/api/docker-compose.dev.yml" \
  || fail "api overlay networks: replaces the base list; keep apps"
grep -q -- '- backend' "$ROOT/stack/api/docker-compose.dev.yml" \
...
if [[ -f "$ROOT/.env.shared" ]] && command -v docker >/dev/null; then
  ...
  project="$(sed -n 's/^COMPOSE_PROJECT_NAME=//p' "$ROOT/.env.shared" | tail -n 1)"
  project="${project:-urantialab}"
  pg_dev="$(make -C "$ROOT/stack/db/postgres" MODE=dev config)" \
    || fail "MODE=dev postgres compose config failed"
  echo "$pg_dev" | grep -q "${project}-backend" \
    || fail "MODE=dev postgres must stay on ${project}-backend"
  echo "$pg_dev" | grep -q "${project}-dev" \
    || fail "MODE=dev postgres must attach ${project}-dev"
  echo "$pg_dev" | grep -q "${project}-apps" \
    && fail "MODE=dev postgres must not join ${project}-apps"
  pg_prod="$(make -C "$ROOT/stack/db/postgres" MODE=prod config)" \
    || fail "MODE=prod postgres compose config failed"
  echo "$pg_prod" | grep -q "${project}-dev" \
    && fail "MODE=prod postgres must not attach ${project}-dev"
fi
